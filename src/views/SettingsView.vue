...
import { BookOpen, ChevronRight, CircleCheck, Database, Download, Laptop, Minus, Moon, Palette, Plus, SlidersHorizontal, Sun, Trash2, Upload } from 'lucide-vue-next'
...
async function previewImport() { try { backupPreview.value = await api.previewBackup() } catch (error) { toast.error(error.message) } }
async function restoreBackup() { try { await api.commitBackup(backupPreview.value.token); backupPreview.value = null; await settings.load(); toast.success('备份恢复完成') } catch (error) { toast.error(error.message) } }
